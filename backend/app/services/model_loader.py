...
class VisionLanguageService:
    ...
    def _build_pipeline(self, model_id: str, model_dir: Path):
        ...
        candidate_tasks = [name for name in ("image-text-to-text", "image-to-text") if name in SUPPORTED_TASKS]
        if not candidate_tasks:
            raise RuntimeError(
                "No multimodal generation task is available in this transformers runtime. "
                f"Available tasks: {sorted(SUPPORTED_TASKS.keys())}"
            )
        ...
        raise RuntimeError(
            f"Unable to initialize multimodal pipeline for {model_id} using tasks {candidate_tasks}: {last_err}"
        )
