...
async function pollJob(jobId) {
  ...
  while (!done) {
    ...
    jobProgressText.textContent = `Status: ${state.status} | ${progress}%`;
    logsBox.textContent = logs.join('\n');

    if (state.status === 'completed') {
      renderResults(Array.isArray(state.results) ? state.results : []);
      done = true;
...
function renderResults(results) {
  resultsWrap.innerHTML = results
    ...
    .map(item => {
      ...
      const keyMessages = Array.isArray(r.key_messages) ? r.key_messages : [];
      return `
      <article class="result-card">
        ...
        <div class="result-body">
          ...
          <p><strong>Key messages:</strong> ${keyMessages.join(' | ') || '-'}</p>
          <p>
